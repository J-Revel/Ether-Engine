...
Event_Callback :: proc(e: Event) -> bool
...
Event :: struct {
	...
	using data: struct #raw_union #align 8 {
		...
		key: struct {
			...
			key_len:  i32,
			code_len: i32,
			key_buf:  [KEYBOARD_MAX_KEY_SIZE]byte,
			code_buf: [KEYBOARD_MAX_KEY_SIZE]byte,
		},
	...
	user_data: rawptr,
	callback:  proc(e: Event) -> bool,
	event_captured: bool,
}

read_key_code :: proc(e: ^Event) -> string {
	return string(e.key.code_buf[:e.key.code_len])
}

read_key_key :: proc(e: ^Event) -> string {
	return string(e.key.key_buf[:e.key.key_len])
}
...
add_event_listener :: proc(id: string, kind: Event_Kind, user_data: rawptr, callback: Event_Callback, use_capture := false) -> bool {
	result_ptr := new(i32)
	result_ptr^ = 0
	@(default_calling_convention="contextless")
	foreign dom_lib {
		@(link_name="add_event_listener")
		_add_event_listener :: proc(id: string, name: string, name_code: Event_Kind, user_data: rawptr, callback: proc "odin" (Event) -> bool, use_capture: bool, capture_result: ^i32) -> bool ---
	}
	// TODO: Pointer_Lock_Change etc related stuff for all different browsers
	return _add_event_listener(id, event_kind_string[kind], kind, user_data, callback, use_capture, result_ptr)
}

remove_event_listener :: proc(id: string, kind: Event_Kind, user_data: rawptr, callback: Event_Callback) -> bool {
	@(default_calling_convention="contextless")
	foreign dom_lib {
		@(link_name="remove_event_listener")
		_remove_event_listener :: proc(id: string, name: string, user_data: rawptr, callback: proc "odin" (Event) -> bool) -> bool ---
	}
	return _remove_event_listener(id, event_kind_string[kind], user_data, callback)
}

add_window_event_listener :: proc(kind: Event_Kind, user_data: rawptr, callback: Event_Callback, use_capture := false) -> bool {
	@(default_calling_convention="contextless")
	foreign dom_lib {
		@(link_name="add_window_event_listener")
		_add_window_event_listener :: proc(name: string, name_code: Event_Kind, user_data: rawptr, callback: proc "odin" (Event) -> bool, use_capture: bool, capture_result: ^i32) -> bool ---
	}
	result_ptr := new(i32)
	result_ptr^ = 0

	return _add_window_event_listener(event_kind_string[kind], kind, user_data, callback, use_capture, result_ptr)
}

remove_window_event_listener :: proc(kind: Event_Kind, user_data: rawptr, callback: proc(e: Event) -> bool) -> bool {
	@(default_calling_convention="contextless")
	foreign dom_lib {
		@(link_name="remove_window_event_listener")
		_remove_window_event_listener :: proc(name: string, user_data: rawptr, callback: proc "odin" (Event) -> bool) -> bool ---
	}
...
add_custom_event_listener :: proc(id: string, name: string, user_data: rawptr, callback: proc(e: Event), use_capture := false) -> bool {
	...
	foreign dom_lib {
		@(link_name="add_event_listener")
		_add_event_listener :: proc(id: string, name: string, name_code: Event_Kind, user_data: rawptr, callback: proc "odin" (Event), use_capture: bool, capture_result: ^i32) -> bool ---
	}
	result_ptr := new(i32)
	result_ptr^ = 0
	return _add_event_listener(id, name, .Custom, user_data, callback, use_capture, result_ptr)
}
...
do_event_callback :: proc(user_data: rawptr, callback: proc(e: Event) -> bool, result: ^u32) {
	@(default_calling_convention="contextless")
	foreign dom_lib {
	...
	if callback != nil {
		...
		if(event.kind == .Key_Press){
			fmt.println("key :", read_key_key(&event))
			fmt.println("code :", read_key_code(&event))
		}
		result^ = 0
		if callback(event) {
			result^ = 1
