...
{
    texture_it := util.table_iterator(&textures)
    loaded_texture_handle: Texture_Handle
    for texture_data, texture_handle in util.table_iterate(&texture_it)
    {
    ...
    if texture_load_ok
    {
        texture_handle, ok := util.table_add(&textures, loaded_texture_data)
        assert(ok)
        return texture_handle, true
    }
    else
...
{
    sprite_it := util.table_iterator(&sprites)
    for sprite, sprite_handle in util.table_iterate(&sprite_it)
    {
...
get_or_load_sprite :: proc(using db: ^Sprite_Database, asset: Sprite_Asset) -> (Sprite_Handle, bool)
{
    ...
    if load_ok
    {
        ...
        for loaded_sprite_name, index in loaded_sprites_names
        {
            ...
            new_sprite_handle, add_ok := util.table_add(&sprites, new_sprite)
            assert(add_ok)
...
save_sprites_to_file :: proc(path: string, sprites_ids: []Sprite_Handle) -> os.Errno
{
    ...
    {
        sprite := util.handle_get(sprite_id)
    ...
    for sprite in sorted_sprites
    {
        if sprite.texture.id != current_texture.id
        {
            ...
            texture := util.handle_get(sprite.texture)
        ...
        if marshal_error == nil do os.write_string(file_handle, strings.string_from_ptr(&encoded[0], len(encoded)))
...
load_sprites_from_file :: proc (path: string, textures: ^util.Table(Texture), sprites: ^util.Table(Sprite)) -> bool
{
    ...
    if ok
    {
        ...
        for texture_path, sprite_list in parsed_object
        {
            ...
            assert(ok)
            texture_id, texture_add_ok := util.table_add(textures, texture)
    ...
    color: Color)
{
    ...
    texture_data := util.handle_get(texture)
    ...
    if texture != render_system.current_texture
    {
        ...
        if index_count > 0
        {
            ...
            if util.is_valid(render_system.current_texture)
            {
...
render_sprite_buffer_content :: proc(render_system: ^Sprite_Render_System, camera: ^Camera, viewport: Viewport)
{
    ...
    {
		if util.is_valid(pass.texture) do texture_id = util.handle_get(pass.texture).texture_id
		gl.BindTexture(gl.TEXTURE_2D, texture_id)
    ...
    if util.is_valid(render_system.current_texture) do texture_id = util.handle_get(render_system.current_texture).texture_id
    gl.BindTexture(gl.TEXTURE_2D, texture_id)
	prepare_buffer_render(&render_system.render_state, viewport)
	use_camera(render_system, camera)
...
render_ui_buffer_content :: proc(render_system: ^Sprite_Render_System, viewport: Viewport)
{
    ...
    for pass, index in render_system.passes
    {
		texture_id := util.is_valid(pass.texture) ? util.handle_get(pass.texture).texture_id : render_system.render_state.default_texture
        gl.BindTexture(gl.TEXTURE_2D, texture_id)
		prepare_buffer_render(&render_system.render_state, viewport)
        render_buffer_content_part(&render_system.render_state, index_cursor, pass.index_count)
		cleanup_buffer_render()
	...
	texture_id := util.is_valid(render_system.current_texture) ? util.handle_get(render_system.current_texture).texture_id : render_system.render_state.default_texture
    gl.BindTexture(gl.TEXTURE_2D, texture_id)
	prepare_buffer_render(&render_system.render_state, viewport)
	render_buffer_content_part(
		&render_system.render_state, 
...
{
    util.table_init(&textures, texture_cap)
    util.table_init(&sprites, sprite_cap)
}
