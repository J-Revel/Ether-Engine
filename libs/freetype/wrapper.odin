...
convert_error_code :: proc(code: Error_Code) ->Error 
{
	return cast(Error)code;
}

init :: proc(library: ^Library) -> Error
{
	return convert_error_code(Init_FreeType(library));
}

done :: proc(library: Library) -> Error
{
	return convert_error_code(Done_FreeType(library));
}


new_face :: #force_inline proc(library: Library, filepathname: string, face_index: Long, aface: ^Face) -> Error 
{
	return convert_error_code(New_Face(library, strings.clone_to_cstring(filepathname, context.temp_allocator), face_index, aface));
}

new_memory_face :: #force_inline proc(library: Library, file_base: ^byte, file_size: Long, face_index: Long, aface: ^Face) -> Error
{
	return convert_error_code(New_Memory_Face(library, file_base, file_size, face_index, aface));
}

free_face :: #force_inline proc(face: Face) -> Error
{
	return convert_error_code(Done_Face(face));
}


set_char_size :: #force_inline proc(face: Face, char_width, char_height: i32, horz_resolution, vert_resolution: u32) -> Error
{
	return convert_error_code(Set_Char_Size(face, char_width, char_height, horz_resolution, vert_resolution));
}

set_pixel_sizes :: #force_inline proc(face: Face, pixel_width, pixel_height: u32) -> Error
{
	return convert_error_code(Set_Pixel_Sizes(face, pixel_width, pixel_height));
}
...
{
	return convert_error_code(Get_Kerning(face, left_glyph, right_glyph, kern_mode, akerning));
}

load_char :: #force_inline proc(face: Face, char_code: u32, load_flags: i32) -> Error
{
	return convert_error_code(Load_Char(face, char_code, load_flags));
}

load_glyph :: #force_inline proc(face: Face, glyph_index: u32, load_flags: i32) -> Error
{
	return convert_error_code(Load_Glyph(face, glyph_index, load_flags));
}

select_charmap :: #force_inline proc(face: Face, encoding: Encoding) -> Error
{
	return convert_error_code(Select_Charmap(face, encoding));
}
...
{
	return convert_error_code(Load_Sfnt_Table(face, tag, offset, buffer, length));
}
